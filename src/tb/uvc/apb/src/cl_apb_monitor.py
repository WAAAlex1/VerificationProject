...
class cl_apb_monitor(uvm_monitor):
    ...
    async def handle_reset(self):
        while True:
            ...
            if (self.cfg.active_low_reset and self.cfg.vif.rst.value.binstr == '0') or \
            (not self.cfg.active_low_reset and self.cfg.vif.rst.value.binstr == '1') :
    ...
    async def monitor_loop(self):
        ...
        while True:
            ...
            self.ap.write(item)
            self.logger.debug(f"Item sent from monitor: \n{item}")
